...
def test_api():
    ...
    params = {"page": 1, "per_page": 10}
    response = requests.get(f"{BASE_URL}/games/genre/Adventure", params=params)
    print(f"Status: {response.status_code}")
    ...
    response = requests.get(f"{BASE_URL}/games", params={"page": 1, "per_page": 1})
    games = response.json()
    if games:
        game = games[0]
    ...
    response = requests.get(f"{BASE_URL}/games/genre/Adventure", params={"page": 1, "per_page": 1})
    games = response.json()
    if games:
        game = games[0]
